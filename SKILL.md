...
| 内容发现路径 | 启动时检测 MP 三前置条件（Edge/session/wechat-ocr-research），确定路径后明确告知用户 | 不应静默降级到 WebSearch 而不告知路径变化及影响 |
...
| 条件 | 检查方式 | 缺失影响 |
|------|---------|---------|
| Edge 浏览器 | Mac 下检查 `~/Library/Application Support/Microsoft Edge/Default/Cookies` | ✗ → MP 不可用 |
| MP 登录态 | 检查 `cache/session.json` 存在且未过期 | ✗ → MP 不可用 |
| wechat-ocr-research | 检查 skill 是否已安装 | ✗ → MP 不可用 |

Agent 输出要求（三条件任一缺失时，在回复中明确声明）：

```
⚠️ MP 自动发现未启用（缺少: <具体缺什么>），将使用 WebSearch 构建候选。
影响：① 文章链接可能来自转载站而非微信原文 ② 摘要是搜索结果片段而非原文节选。
如需要 MP 直连，请先完成 Level 3 配置（见 references/mp-setup-guide.md）。
```
...
| 优先级 | 路径 | 来源 | 失败处理 |
|--------|------|------|---------|
| 1 | MP 可用 | MP API 返回的 `digest` 字段 | — |
| 2 | WebSearch | 搜索引擎返回的 `snippet` | 若空 → 回退到 3 |
...
## 标准执行流程（前置检查 → 四号 MP → 八判据评分 → 三层交付）

> 2026-07-28 确立的标准执行链路。每次真实周报按此流程：前置检查 → Session → 拉取 → 构建 → 评分 → 交付。

### Step 0: 执行前置检查（强制，不可跳过）

Agent 必须先完成「执行前置检查」段落的 MP 可用性检测 + 路径声明。
基于结果走后续分支：

```
MP 三条件全 ✓ → Step 1 (MP 路径，最佳质量)
      任一 ✗ → Agent 声明降级 → 跳过 Step 1-2，走 WebSearch 构建候选
```
